...
\usepackage[scheme=plain,fontset=fandol,UTF8]{ctex}
...
\midrule
\endfirsthead
\caption[]{Per-language performance metrics for the models used (continued).}\\
\toprule
...
\label{fig:sub6}
\end{subfigure}
\caption{Correlation between English psycholinguistic norms  and type-level groundedness  (left) or uncertainty coefficient (right): i.e., the average ratio between LM surprisal and captioning model surprisal. Type-level measures were computed by averaging scores across the COCO-dev dataset for types which occur at least 30 times.}
...
\bibliography{Thesis2,anthology}
